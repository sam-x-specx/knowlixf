...
export default function Navbar() {
  ...
  return (
    <>
      <nav className="fixed top-0 left-0 right-0 z-40 border-b border-white/[0.06] bg-[#030712]/80 backdrop-blur-md">
        <div className="max-w-6xl mx-auto px-4 sm:px-6">
          <div className="flex items-center justify-between h-14 sm:h-16">
            ...
            {session && (
              <div className="hidden sm:flex items-center gap-3">
                <div className="flex items-center gap-2">
                  {session.user?.image ? (
                    <Image src={session.user.image} alt="avatar" width={32} height={32} className="rounded-full ring-1 ring-white/10" />
                  ) : (
                    <div className="w-8 h-8 rounded-full bg-indigo-600 flex items-center justify-center text-white text-xs font-bold">
                      {session.user?.name?.[0] ?? "?"}
                    </div>
                  )}
                  <span className="text-sm text-zinc-300 font-medium">{session.user?.name?.split(" ")[0] || session.user?.email}</span>
                </div>
                <button
                  onClick={() => signOut()}
                  className="px-3 py-1.5 rounded-lg border border-white/[0.08] bg-white/[0.04] hover:bg-white/[0.08] text-zinc-400 hover:text-white text-xs font-medium transition-all"
                >
                  Sign out
                </button>
              </div>
            ...
            {session ? (
              <>
                <div className="flex items-center gap-3 px-4 py-3 rounded-xl border border-white/[0.06] bg-white/[0.03]">
                  {session.user?.image ? (
                    <Image src={session.user.image} alt="avatar" width={36} height={36} className="rounded-full ring-1 ring-white/10" />
                  ) : (
                ...
                <button
                  ...
                  className="w-full px-4 py-3 rounded-xl border border-white/[0.08] bg-white/[0.04] text-zinc-300 text-sm font-medium transition-all text-left"
                >
                  Sign out
                </button>
              </>
