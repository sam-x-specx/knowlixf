...
// // // ── Quiz constants (no tokens) ──────────────────────────────────────────────
// // const FREE_LIMIT = 5;           // max questions for non-logged-in users
// // const MAX_QUESTIONS = 70;       // max questions for logged-in users
// // const PAGE_SIZE = 35;           // bars shown per page in the grid
// // // ─────────────────────────────────────────────────────────────────────────────

// // type UpgradeReason = "login_required" | "free_limit";
...
// //           <div>
// //             <h2 className="text-xl sm:text-2xl font-black text-white tracking-tight">
// //               {isLoginRequired ? "Sign in to Continue" : "Unlock More Questions"}
// //             </h2>
// //             <p className="text-zinc-400 text-sm mt-2 leading-relaxed">
// //               {isLoginRequired
// //                 ? `Create a free account to generate up to ${MAX_QUESTIONS} questions per quiz and save your progress.`
// //                 : `Generate up to ${MAX_QUESTIONS} questions per quiz with a free account. Sign in now!`}
// //             </p>
// //           </div>
...
// //               ["🏆", "Save quiz results & review answers"],
// //               ["💾", "Export & share your quizzes"],
// //             ].map(([icon, text]) => (
// //               <div key={String(text)} className="flex items-center gap-3 text-zinc-300">
// //                 <span>{icon}</span>
// //                 <span>{text}</span>
...
// //   // Logged-in users get MAX_QUESTIONS, non-logged-in get FREE_LIMIT
// //   const effectiveMax = isLoggedIn ? MAX_QUESTIONS : FREE_LIMIT;
...
// //   const handleQuestionCountChange = (val: number) => {
// //     // Non-logged-in users can't exceed FREE_LIMIT
// //     if (!isLoggedIn && val > FREE_LIMIT) {
// //       openUpgrade("login_required");
// //       return;
// //     }
...
// //     // Gate: must be logged in for more than FREE_LIMIT questions
// //     if (!isLoggedIn && questionCount > FREE_LIMIT) { 
// //       openUpgrade("login_required"); 
// //       return; 
// //     }
...
// //             <div className="inline-flex items-center gap-2 px-3 sm:px-4 py-1.5 rounded-full border border-indigo-500/30 bg-indigo-500/10 text-indigo-300 text-xs sm:text-sm font-medium mb-5 sm:mb-8 tracking-wide">
// //               <span className="w-1.5 h-1.5 rounded-full bg-indigo-400 animate-pulse shrink-0" />
// //               {isLoggedIn ? "AI-POWERED QUIZ ENGINE" : "AI-POWERED QUIZ ENGINE"}
// //             </div>
...
// //             <div className="mt-4 flex flex-wrap items-center justify-center gap-2">
// //               {isLoggedIn ? (
// //                 <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full border border-emerald-500/30 bg-emerald-500/10 text-emerald-300 text-xs font-semibold">
// //                   <span>✓</span>
// //                   <span>Logged in · Up to {MAX_QUESTIONS} questions</span>
// //                 </div>
// //               ) : (
// //                 <button
// //                   onClick={() => openUpgrade("login_required")}
...
// //                 >
// //                   <span>🔐</span>
// //                   <span>Sign in to unlock {MAX_QUESTIONS} questions</span>
// //                 </button>
// //               )}
// //             </div>
// //           </div>
...
// //                 {visibleBars.map((n) => {
// //                   const isSelected = n <= questionCount;
// //                   const isAvailable = isLoggedIn || n <= FREE_LIMIT;
...
// //                         handleQuestionCountChange(n);
// //                       }}
// //                       title={`${n} question${n > 1 ? "s" : ""}${!isAvailable ? " · Sign in required for more" : ""}`}
// //                       className={buttonClass}
// //                     />
// //                   );
// //                 })}
...
// //                   <div className="flex items-center gap-1.5">
// //                     <span className="w-2 h-2 rounded-sm bg-violet-600" />
// //                     <span className="text-[10px] text-zinc-500">Premium ({FREE_LIMIT + 1}–{MAX_QUESTIONS})</span>
// //                   </div>
// //                 </div>
// //                 <div className="flex items-center gap-1">
// //                   <button
...
// //                 <>
// //                   <span className="w-px h-3 bg-zinc-700 hidden sm:block" />
// //                   <span className="text-amber-400">⚠️ Sign in for {questionCount} questions</span>
// //                 </>
// //               )}
// //             </div>
...
// //               { icon: "🔗", title: "Any Link", desc: "YouTube, Wikipedia, articles" },
// //               { icon: "📄", title: "PDF Upload", desc: "Notes, textbooks, docs" },
// //               { icon: "📊", title: "Track Progress", desc: "Save & review your history" },
// //             ].map((f) => (
// //               <div key={f.title} className="text-center px-1">
// //                 <div className="text-xl sm:text-2xl mb-1 sm:mb-2">{f.icon}</div>
// //                 <div className="text-xs font-semibold text-zinc-300">{f.title}</div>
...
// type UpgradeReason = "login_required";
...
//         <div className="flex flex-col items-center text-center gap-4">
//           <div className="w-14 h-14 sm:w-16 sm:h-16 rounded-2xl flex items-center justify-center text-2xl sm:text-3xl shadow-lg bg-gradient-to-br from-indigo-600 to-blue-600 shadow-indigo-900/50">
//             🔐
//           </div>
...
//   const handleQuestionCountChange = (val: number) => {
//     // Don't allow selection if not logged in and trying to exceed FREE_LIMIT
//     if (!isLoggedIn && val > FREE_LIMIT) {
//       openUpgrade("login_required");
//       return;
//     }
...
//     const clamped = Math.min(effectiveMax, Math.max(1, val));
//     console.log(`Setting question count to: ${clamped}`); // Debug log
//     setQuestionCount(clamped);
//     setCurrentPage(clamped > PAGE_SIZE ? 1 : 0);
//   };
...
//     // CRITICAL FIX: Validate question count based on login status
//     if (!isLoggedIn && questionCount > FREE_LIMIT) { 
//       openUpgrade("login_required"); 
//       return; 
//     }
...
//     if (isLoggedIn && questionCount > MAX_QUESTIONS) {
//       setError(`Maximum ${MAX_QUESTIONS} questions allowed`);
//       return;
//     }

//     console.log(`Submitting quiz with ${questionCount} questions`); // Debug log
...
//             url: url.trim(), 
//             difficulty, 
//             questionCount  // Make sure this is sending the correct value
//           }),
//         });
//       }
...
// Free users: up to 5 questions. Logged-in users: up to 70.
const FREE_LIMIT = 5;
const MAX_QUESTIONS = 70;
const PAGE_SIZE = 35;
        ...
        <div className="text-center mb-8 sm:mb-12 lg:mb-16">
          ...
          <div className="mt-4 flex flex-wrap items-center justify-center gap-2">
            {isLoggedIn ? (
              <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full border border-emerald-500/30 bg-emerald-500/10 text-emerald-300 text-xs font-semibold">
                <span>✓</span>
                <span>Signed in · Up to {MAX_QUESTIONS} questions</span>
              </div>
            ) : (
              <button
          ...
          <div>
              ...
              {visibleBars.map((n) => {
                ...
                return (
                  <button
                    ...
                    title={`${n} question${n > 1 ? "s" : ""}${!isAvailable ? " · Sign in required" : ""}`}
                    className={buttonClass}
                  />
                );
              })}
            ...
            <div className="flex items-center justify-between mt-2">
              <div className="flex items-center gap-3">
                ...
                <div className="flex items-center gap-1.5">
                  <span className="w-2 h-2 rounded-sm bg-violet-600" />
                  <span className="text-[10px] text-zinc-500">Sign in ({FREE_LIMIT + 1}–{MAX_QUESTIONS})</span>
                </div>
              </div>
              <div className="flex items-center gap-1">
                <button
